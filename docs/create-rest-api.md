...
我们有一个包 `marucat_app`，我们的代码基本放在这里。

这个包之外还有一个 `run.py`，我们定义其为程序的入口，启动的位置。

为了方便调试，我们在 `run.py` 中以 DEGUB 模式启动开发服务器。
...
使用 Flask 框架，我们需要先实例化一个 Flask 对象，然后我们要做的就是对这个对象绑定路由，决定返回什么，除此之外的细节都由框架帮我们处理了。
...
首先我们从 flask 包中导入 Flask 类，从 json 包导入 dumps 函数。

我们创建一个 Flask 对象，我们之后的操作都围绕这个对象进行。

实例化 Flask 对象时需要提供一个识别符作为第一个参数，通常我们传递 `__name__` 作为这个参数。
...
我们在浏览器中打开开发者工具，切换到 Network 标签（Chrome 浏览器下），在这里可以看到我们访问根路径时得到对 Response 对象（如果你没看到，那就在打开开发者工具的情况下再访问一次根路径）。

这是我得到的一个 Response 对象。
...
因为我们使用 dumps 方法将 dict 数据转换成了 JSON 字符串，但它还是字符串！

服务器把它当作普通的字符串处理，这时需要我们手动设置 Content-Type 内容。
