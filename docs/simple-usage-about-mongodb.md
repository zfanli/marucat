...
上面这个例子，我们想得到所有 `views` 值大于等于1000的文章。

MongoDB 中使用 `$gte` 表达大于等于的关系，含义等同于 `>=`。
...
这些操作符在需要表达比较关系的场合都有效，不仅限于 `find` 查询，还包括更新、删除和 `aggregate` 聚合等场合。
...
使用 `$push` 操作符可以将一条评论添加到评论 list 中。
